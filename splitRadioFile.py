...
"""
This file is a rip of the other one.

This one just prints call headers, 
"""
# Project notes
...
outputFilename = "Radio-offsets.txt"
...
def handleCallHeader(offset: int) -> int: # Assume call is just an 8 byte header for now, then \x80, then script length (2x bytes)
    ...
    length = struct.unpack('>H', line[9:11])[0] + 9
    ...
    if debugOutput:
        if line[8].to_bytes() != b'\x80':
            ...
            return False
    """
    output.write(f'Call Header: {humanFreq}, offset = {offset}, length = {length}, UNK0 = {unk0.hex()}, UNK1 = {unk1.hex()}, UNK2 = {unk2.hex()}, Content = {line.hex()}\n')
    return length
...
def main():
    ...
    """
    This version just parses calls, offsets, and splits the files.
