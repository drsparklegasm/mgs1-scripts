...
def translateJapaneseHex(bytestring: bytes, graphicBytes: str) -> str: # Needs fixins, maybe move to separate file?
	i = 0
	messageString = ''
	callDict = makeCallDictionary(bytes.fromhex(graphicBytes))
	while i < len(bytestring) - 1:
		...
		if bytestring[i].to_bytes() == b'\x96':
			...
			try:
				messageString += getRadioCharacter(bytestring[ i : i + 2 ].hex())
			except:
